...
def health_check(request: Request) -> dict[str, str]:
    ...
    # app.state.mqtt lo crea el lifespan; no existe si la app se usa sin arrancarla.
    listener = getattr(request.app.state, "mqtt", None)
    mqtt_status = str(listener.status) if listener is not None else "disabled"
